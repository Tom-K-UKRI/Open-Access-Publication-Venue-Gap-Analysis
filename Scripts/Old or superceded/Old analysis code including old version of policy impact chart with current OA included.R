...
compliance_h_a <- merged_pvga %>%
  ...
  mutate(percent = round(n / sum(n) * 100,1), cml = round(cumsum(percent)), Scenario = "S1 (all gold\nOA supported)")
...
policy_impact_a_bar <- ggplot(policy_impact_a, aes(fill=Compliance_Route, y=percent, x=Scenario)) +
  ...
  scale_fill_manual(values = c("#F08900", "#FBBB10", "#16978A", "#FF5A5A"),
                    name = "Routes to Open Access\nsupported in each\npolicy scenario",
                    labels = c("Gold OA in\nfully OA journal\n", 
                               "Gold OA in hybrid\njournal (with Transitional\nAgreement for S2)",
                               "\nConfirmed green OA\n",
                               "None identified under\ncurrent journal policies\n")) +
...
ggsave(policy_impact_a_bar, filename = "Output/Charts/Impact of each policy scenario (articles)_target_tas.png",
...
compliance_new_ukri_funder <- ukri_funders  %>%
  ...
  select(-c(`total_n_c: hybrid gold with a TA`, `total_n_c: confirmed green oa`, `total_n_not compliant`)) %>%
  relocate(`ukri_funder`,total_supported)
...
compliance_new_ukri_funder_ta <- bind_cols(compliance_new_ukri_funder, TA_ukri_funder) %>%
  select(-c(`ukri_funder...8`,has_ta, n)) %>%
...
# Publisher by discipline long tail
  # this suggests that AHSS doesn't have a particularly long tail
publishers_per_discipline <- disciplines %>%
  filter(!is.na(disc)) %>%
  group_by(disc) %>%
  count(Publisher) %>%
  count(disc)

# Number of articles missing publisher information in each discipline
  # this suggests that AHSS doesn't have lots of articles missing publisher info
missing_publisher_info <- bind_rows(merged_pvga %>% filter(grepl("Arts", discipline)) %>% count(Publisher) %>% mutate(percent = n/sum(n)*100, discipline = "Arts") %>% filter(is.na(Publisher)),
                                    merged_pvga %>% filter(grepl("Health", discipline)) %>% count(Publisher) %>% mutate(percent = n/sum(n)*100, discipline = "Health") %>% filter(is.na(Publisher)),
                                    merged_pvga %>% filter(grepl("Life", discipline)) %>% count(Publisher) %>% mutate(percent = n/sum(n)*100, discipline = "Life") %>% filter(is.na(Publisher)),
                                    merged_pvga %>% filter(grepl("Physical", discipline)) %>% count(Publisher) %>% mutate(percent = n/sum(n)*100, discipline = "Physical") %>% filter(is.na(Publisher)),
                                    merged_pvga %>% filter(grepl("Social", discipline)) %>% count(Publisher) %>% mutate(percent = n/sum(n)*100, discipline = "Social") %>% filter(is.na(Publisher))) %>%
  relocate(discipline) %>%
  select(-Publisher)
