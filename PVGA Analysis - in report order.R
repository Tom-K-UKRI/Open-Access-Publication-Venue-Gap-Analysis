...
##########
# TRANSFORMATIVE AGREEEMENTS

# What number and proportion of articles journals are covered by TAs and how many are only potentially compliant because of this route?
...
compliance_old_ref_a <- ref_panels %>%
  filter(!(is.na(sherpa_id) & has_ta == "no")) %>%
...
compliance_old_ref_j <- ref_panels %>%
  filter(!duplicated(journal_title)) %>%
  filter(!(is.na(sherpa_id) & has_ta == "no")) %>%
  filter(ref != "Missing" & ref != "") %>% 
  group_by(ref) %>%
...
compliance_old_ref <- bind_rows(compliance_old_ref_a, compliance_old_ref_j)
rm(compliance_old_ref_a, compliance_old_ref_j)

openxlsx::write.xlsx(as.data.frame(compliance_old_ref), 'compliance_old_ref.xlsx')



###########
# POTENTIAL COMPLIANCE WITH NEW POLICY
...
# 1a. Potential compliance with new policy (if hybrid gold allowed)
compliance_h_a <- merged_pvga %>%
  filter(!(is.na(sherpa_id) & has_ta == "no")) %>%
  count(compliance_hybrid, .drop = FALSE) %>%
  mutate(percent = (n / sum(n) * 100)) %>%
  mutate(cml = round(cumsum(percent),0)) %>%
  mutate(percent = round(percent,0)) %>%
  adorn_totals("row")
...
openxlsx::write.xlsx(as.data.frame(compliance_h), 'compliance_hybrid_permitted.xlsx')
...
# 6. Compliance by REF panel (# I don't think this analysis is possible at journal level since a lot of journals publish across different REF panels)

ref_panels <- merged_pvga %>% pivot_longer(c(ref_panel, ref_panel2), names_to = "ref_all", values_to ="ref")

compliance_ref_a <- ref_panels %>%
  filter(!(is.na(sherpa_id) & has_ta == "no")) %>%
  filter(ref != "Missing" & ref != "") %>% 
  group_by(ref) %>%
...
compliance_ref <- bind_rows(compliance_ref_a, compliance_ref_j)
rm(compliance_ref_a, compliance_ref_j)

openxlsx::write.xlsx(as.data.frame(compliance_ref), 'compliance_ref.xlsx')

  # Tree map for Compliance by REF panel 
compliance_ref_chart <- compliance_ref %>%
  group_by(ref, compliance3) %>%
  head(16) %>%
  summarise_all(funs(sum))

library(treemap)

compliance_ref_tree_refa <- compliance_ref_chart %>%
  filter(ref == "A") %>%
  treemap(index="compliance3",
          vSize="percent",
          type="index",
...
compliance_ref_tree_refb <- compliance_ref_chart %>%
  filter(ref == "B") %>%
  treemap(index="compliance3",
          vSize="percent",
          type="index",
...
compliance_ref_tree_refc <- compliance_ref_chart %>%
  filter(ref == "C") %>%
  treemap(index="compliance3",
          vSize="percent",
          type="index",
...
compliance_ref_tree_refd <- compliance_ref_chart %>%
  filter(ref == "D") %>%
  treemap(index="compliance3",
          vSize="percent",
          type="index",
